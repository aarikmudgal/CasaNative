...
Use a Conventional Commit pull-request title or squash-merge message so automated versioning can classify the change:

- `feat: ...` or `feat(scope): ...` selects a minor release.
- `fix: ...`, `docs: ...`, `chore: ...`, and other non-release commits select a patch release.
- Add `!` before the colon or a `BREAKING CHANGE:` footer only for an intentional incompatible change; this selects a major release.

The highest bump across all commits after the latest release tag wins. Commits beginning with `chore(release):` are generated by automation and do not cause another release. Authored `[Unreleased]` notes are strongly preferred for user-visible nuance; if they are empty, automation generates a `Changed` list from sanitized non-release commit subjects so a release cannot become stranded. Do not manually move `[Unreleased]` into a numbered section or edit Xcode version/build settings for an ordinary pull request; the release workflow performs those changes together after the merged `main` commit passes its checks.

Before requesting review, run the release metadata checks as well as the app tests:

```sh
python3 -m py_compile scripts/release_tool.py
python3 -m unittest discover -s scripts/tests -p 'test_*.py'
python3 scripts/release_tool.py check
python3 scripts/release_tool.py plan --json
```

`check` and `plan` are read-only. A plan requires a canonical reachable `vX.Y.Z` tag, synchronized Xcode versions, and a valid changelog comparison chain. Maintainers should leave `prepare`, the release commit, tag creation, asset upload, and final publication to the Release workflow so its complete validation and atomic-push guarantees remain intact.
