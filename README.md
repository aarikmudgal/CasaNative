...
For a planned release, the workflow reuses the successful exact-commit CI and CodeQL results instead of rerunning simulator tests or static analysis. It promotes `[Unreleased]`, updates both Xcode version configurations, verifies that only the changelog and version metadata changed, resolves the locked packages, and builds the unsigned iPhoneOS Release app. Only after those release-specific checks pass does it create a `chore(release): vX.Y.Z` commit authored and committed as `Aarik Mudgal <aarikmudgal@gmail.com>`, create an annotated tag, and atomically push `main` and the tag. The generated metadata commit carries GitHub's `[skip ci]` instruction because the full merged commit has already passed CI and the final device artifact is built before that commit is pushed. The workflow creates a draft GitHub release, uploads and verifies every asset, and publishes the release only after the draft is complete. It packages:
